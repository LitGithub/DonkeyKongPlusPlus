...
int player::VerticalCollision(int x1, int y1, int w1, int h1, int x2, int y2, int w2, int h2, int ydir) {
	if ((ydir >= 0) &&
		(x1 + w1 > x2) && (x1 < x2 + w2) && (y1 + h1 + 4 > y2) && //the 4 means you're too close to the ground
		(y1 < y2)) { //cout << "adjusting up";
		return 2;
	}
	else if (ydir >= 0 &&
		(x1 + w1 > x2) && (x1 < x2 + w2) && (y1 + h1 + 6 > y2) && //the 6 means you're not too close and don't need adjust the player's position up!
		(y1 < y2)) { //cout << "stable ypos";
		return 1;
	}
	else
		return false;
